...
- Referrer URL (the previous webpage from which you navigated to my site)
...
### Legal Basis
The processing of server log data is based on **Art. 6(1)(f) GDPR** (*legitimate interests*). My legitimate interest lies in the secure, performant, fault-tolerant, and reliable operation of my website infrastructure and defense against cyberattacks.
...
- I have concluded a standard Data Processing Addendum (DPA) with Cloudflare.
...
To ensure rigorous compliance with EU privacy standards, I do not load external third-party tracking scripts directly in your browser upon initial page visit. Instead, consent management is executed directly at the edge layer using **Cloudflare Zaraz Consent Management**.
...
- **EU-Hosting & IP-Masking:** Das Web-Container-Skript wird über meinen serverseitigen Tag Manager (sGTM) auf Servern innerhalb der Europäischen Union ausgeliefert. Hierdurch erfolgt vor Weiterleitung von Anfragen eine Pseudonymisierung bzw. Maskierung der IP-Adresse.
...
I use a **Server-Side Google Tag Manager (sGTM)** container operated via:

**Google Ireland Limited**  
Gordon House, Barrow Street, Dublin 4, Ireland

Unlike traditional client-side tag managers that load dozens of third-party JavaScript libraries directly into your browser, my setup uses **Server-Side Tagging (sGTM Proxy)**:

1. **First-Party Context:** Your browser communicates solely with my own first-party server endpoint rather than establishing direct connections to disparate advertising networks.
2. **Server-Side Sanitization & IP Masking:** Incoming requests are received by my server container first. Before any data payload is forwarded to a downstream tool (such as GA4 or social conversion APIs), IP addresses are truncated, masked, or stripped entirely, and personal query parameters or device identifiers are cleansed.
...
My server container strictly implements **Google Consent Mode v2** in conjunction with Cloudflare Zaraz:
...
- **Dynamic Consent Update:** Only when you actively click to accept specific categories or tools in my consent banner is an updated event (`consent update`) transmitted via the `dataLayer`.
...
All optional analytics and conversion measurement tools are executed **server-side via sGTM** and are individually selectable in my consent manager. **No tool is activated in advance.** Processing occurs exclusively after your explicit opt-in under **Art. 6(1)(a) GDPR**.
...
- **Purpose:** Measurement of campaign performance, referral evaluation, and conversion tracking for my photography content shared on Pinterest.
- **Mechanism:** Communication occurs server-to-server via the Pinterest Conversions API through my server proxy. No direct, unmanaged client-side Pinterest scripts run on your device.
- **Legal Basis:** Art. 6(1)(a) GDPR (Consent).
- **Official Documentation & Links:**
...
- **Mechanism:** Event measurement is routed via Meta's Conversions API (CAPI) through my server-side container. This eliminates third-party tracker injection in your browser and ensures that payloads are filtered and pseudonymized before dispatch.
- **Legal Basis:** Art. 6(1)(a) GDPR (Consent).
- **Official Documentation & Links:**
...
On select tutorial and review articles, I embed informative videos hosted on YouTube (Google Ireland Limited / YouTube LLC).

To safeguard your privacy:
- **No Automatic Connection:** I implement a **2-click content blocker** (privacy-enhanced embedding). When you load a page containing a video, no connection to YouTube or Google servers is established, and no cookies or trackers are set on your device. Only a local placeholder or thumbnail stored on my own infrastructure is displayed.
...
My website provides interactive features including blog article comments and star ratings/feedback. These services are powered by custom serverless Cloudflare Workers:

- **Comments:** When you post a comment on a blog post, the data you submit (name/pseudonym, email address, comment text) is transmitted via encrypted HTTPS to my worker API. The submitting IP address and timestamp are processed solely for the duration necessary to prevent automated spam and abuse. I do not use third-party comment networks (such as Disqus).
...
This site uses **Transport Layer Security (TLS/HTTPS)** encryption across all pages (data protection by design and default, Art. 25(1) GDPR). All traffic between your browser and my edge servers is encrypted using modern cryptographic protocols (TLS 1.3). You can verify the active encryption by checking for the padlock icon in your browser's address bar and the `https://` protocol prefix.
...
- **Right to Revoke Consent (Art. 7(3) GDPR):** Where processing is based on your consent (Art. 6(1)(a) GDPR), you may revoke that consent at any time with effect for the future (e.g., via the consent settings in the footer).
...
To exercise any of these rights, please contact me at [hello@nocamerabag.com](mailto:hello@nocamerabag.com).
